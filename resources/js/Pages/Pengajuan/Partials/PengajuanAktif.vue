...
<template>
    <PengajuanTable
        :items="pengajuanAktif"
        empty-title="Tidak ada pengajuan aktif"
        empty-message="Pengajuan yang sedang diproses akan muncul di sini"
    >
        <template #actions="{ item }">
            ...
            <div class="md:hidden flex space-x-1">
                ...
                <!-- Cancel Button -->
                <Tooltip v-if="item.status === 'pending'" text="Batalkan Pengajuan">
                    <button
                        @click="$emit('cancel', item)"
