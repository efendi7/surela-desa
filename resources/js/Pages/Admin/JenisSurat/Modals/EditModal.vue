...
watch(() => props.surat, (newSurat) => {
    if (newSurat && Object.keys(newSurat).length > 0) {
        form.defaults({
            nama_surat: newSurat.nama_surat || '',
            deskripsi: newSurat.deskripsi || '',
            syarat: Array.isArray(newSurat.syarat) ? [...newSurat.syarat] : [],
        }).reset();
    }
...
const updateSurat = () => {
    ...
    form.put(route('admin.jenis-surat.update', props.surat.id), {
        preserveScroll: true,
        onSuccess: () => {
            closeModal();
        },
...
<template>
    <Modal :show="show" @close="closeModal" max-width="2xl">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-medium text-gray-900">Edit Jenis Surat</h2>
            <p class="mt-1 text-sm text-gray-600">
                Ubah detail surat yang diperlukan.
            </p>
        </div>
        ...
        <div class="px-6 py-4 flex-1 overflow-y-auto">
            <div class="space-y-4">
                <!-- Nama Surat -->
                <div>
                    <InputLabel for="edit_nama_surat" value="Nama Surat" />
                    <TextInput 
                        id="edit_nama_surat" 
                        type="text" 
                ...
                <!-- Deskripsi -->
                <div>
                    <InputLabel for="edit_deskripsi" value="Deskripsi" />
                    <TextInput 
                        id="edit_deskripsi" 
                        type="text" 
                        class="mt-1 block w-full" 
                ...
                <!-- Syarat-syarat -->
                <div>
                    <InputLabel for="edit_syarat" value="Syarat-syarat" />
                    <div class="relative">
                        <button 
                            type="button" 
                    ...
                    <!-- Selected Tags -->
                    <div v-if="form.syarat && form.syarat.length > 0" class="mt-2 flex flex-wrap gap-2">
                        <span 
