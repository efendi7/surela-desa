...
<template>
    <aside
        class="fixed left-0 top-0 w-64 bg-gradient-to-b from-gray-800 to-gray-900 text-white flex flex-col h-screen shadow-xl z-20"
    >
            ...
            >
                <div class="relative flex-shrink-0">
                    <img
                        ...
                        @error="
                            $event.target.src = 'https://placehold.co/48x48/3B82F6/ffffff?text=🏛️'
                        "
                    />
                    <div
                ...
                <div class="flex-1 min-w-0">
                    <div
                        class="font-bold text-white text-base truncate group-hover:text-blue-200 transition-colors duration-200"
                    >
                        {{ profilDesa?.nama_desa || 'SURELA Desa' }}
                    </div>
                    <div
                        v-if="locationDisplay"
                        class="text-xs text-gray-300 truncate mt-1 opacity-80"
                    >
                        {{ locationDisplay }}
                    </div>
                </div>
        ...
        <nav
            class="flex-1 overflow-y-auto scrollbar-thin scrollbar-track-gray-800 scrollbar-thumb-gray-600 hover:scrollbar-thumb-gray-500"
        >
                ...
                <div v-if="user?.role === 'admin'" class="pt-6 mt-6 border-t border-gray-700/50">
                    ...
                    <div class="space-y-1">
                        <NavLink
                            ...
                            class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg hover:bg-gray-700/50 hover:text-white transition-all duration-200"
                            :class="
                                route().current('admin.profil-desa.*')
                        ...
                        <NavLink
                            ...
                            class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg hover:bg-gray-700/50 hover:text-white transition-all duration-200"
                            :class="
                                route().current('admin.jenis-surat.*')
                                    ? 'bg-orange-600 text-white shadow-lg'
                                    : 'text-gray-300'
                            "
                        >
                        ...
                        <NavLink
                            :href="route('admin.proses.index')"
                            :active="route().current('admin.proses.*')"
                            theme="dark"
                            class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg hover:bg-gray-700/50 hover:text-white transition-all duration-200"
                            :class="
                                route().current('admin.proses.*')
                                    ? 'bg-orange-600 text-white shadow-lg'
                                    : 'text-gray-300'
                            "
                        >
                        ...
                        <NavLink
                            ...
                            :active="route().current('admin.pengajuan.riwayat')"
                            theme="dark"
                            class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg hover:bg-gray-700/50 hover:text-white transition-all duration-200"
                            :class="
                                route().current('admin.pengajuan.riwayat')
                                    ? 'bg-orange-600 text-white shadow-lg'
                                    : 'text-gray-300'
                            "
                        >
